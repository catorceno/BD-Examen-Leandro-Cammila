...
def add_date_field(window, row):
    """"""
    Añade al `window` un campo de Fecha Inicio con checkbox "Usar fecha de hoy".
    Devuelve (fecha_var, usar_hoy).
    """"""
    fecha_var = tk.StringVar()
    tk.Label(window, text="Fecha Inicio").grid(
        row=row, column=0, padx=10, pady=5, sticky="e"
    )
    entry = tk.Entry(window, textvariable=fecha_var)
    entry.grid(row=row, column=1, padx=10, pady=5)
...
def add_discount_field(window, row, conn):
    """"""
    Añade al `window` un Combobox de descuentos activos y checkbox "Sin Descuento".
    Devuelve (var_descuento, usar_sin_descuento).
    """"""
    try:
        cursor = conn.cursor()
        cursor.execute("SELECT Nombre FROM DESCUENTOS WHERE Estado = 'Activo'")

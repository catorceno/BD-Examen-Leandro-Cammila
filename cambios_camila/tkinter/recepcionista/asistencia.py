...
def agregarAsistencia(conn):
    window = tk.Toplevel()
    #config_window.centrar(window, 350, 300)
    window.title("Agregar Asistencia")
    ...
    fecha_var, usar_hoy = add_date_field(window, 2, label_text="Fecha")

    # Hora Ingreso
    tk.Label(window, text="Hora Ingreso").grid(row=4, column=0, padx=10, pady=5, sticky="e")
    hora_var = tk.StringVar()
    tk.Entry(window, textvariable=hora_var).grid(row=4, column=1, padx=10, pady=5)
    ...
    def on_save():
        try:
            ...
            hora_txt = hora_var.get()
            # Validar formato HH:MM
            try:
                datetime.strptime(hora_txt, "%H:%M")
            except ValueError:
                raise ValueError("La hora debe tener formato HH:MM (p.ej. 14:30).")

            cursor = conn.cursor()
            cursor.execute(
                "EXEC sp_agregar_asistencia ?, ?, ?, ?",
                ci, servicio, fecha, hora_txt
            )
            conn.commit()
            messagebox.showinfo("Éxito", "Asistencia registrada correctamente.")
            window.destroy()
